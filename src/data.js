...
export function initData(sourceData) {
    ...
    const getIndexes = async () => {
        if (!sellers || !customers) {
            //если индексы еще не установлены, то делаем запросы
    ...
    const getRecords = async (query, isUpdated = false) => {
        const qs = new URLSearchParams(query); //преобразуем объект в SearchParams объект, предст query часть url
        const nextQuery = qs.toString(); //приводим к строковому виду

        if (lastQuery === nextQuery && !isUpdated) {
            //isUpdared параметр нужен,чтобы иметь возможность делать запрос без кеша
            return lastResult; //если параметры запроса не поменялись, то отдаем сохраненные ранее данные
        }
        ...
        lastQuery = nextQuery; //сохраняем для след запросов
        lastResult = {
            total: records.total,
